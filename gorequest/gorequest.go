...
func (s *SuperAgent) SetGinContext(context context.Context) *SuperAgent {
	s.context = context

	return s
}

func (s *SuperAgent) SetTrace(tr opentracing.Tracer) *SuperAgent {
	s.Tracer = tr
	return s
}
...
func (s *SuperAgent) getResponseBytes() (Response, []byte, []error) {
	...
	// tracer request
	if span := s.clientTracer(req); span != nil {
		defer span.Finish()
	}
...
func (s *SuperAgent) clientTracer(req *http.Request) opentracing.Span {
	if s.context == nil {
		return nil
	}

	req.WithContext(s.context)

	if span := opentracing.SpanFromContext(s.context); span != nil {
		span := s.Tracer.StartSpan(fmt.Sprintf("HTTP %s: %s", s.Method, req.URL.Path), opentracing.ChildOf(span.Context()))

		span.SetTag("http.method", s.Method)
		span.SetTag("http.url", req.URL)

		ext.Component.Set(span, defaultComponentName)
		ext.SpanKindRPCClient.Set(span)

		s.context = opentracing.ContextWithSpan(s.context, span)

		err := s.Tracer.Inject(span.Context(), opentracing.HTTPHeaders, opentracing.HTTPHeadersCarrier(req.Header))
		if err != nil {
